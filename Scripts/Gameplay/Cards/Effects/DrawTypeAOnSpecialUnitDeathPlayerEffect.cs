...
{
    /// <summary>
    /// Player effect that draws Type A cards when a special unit dies.
    /// </summary>
    public class DrawTypeAOnSpecialUnitDeathPlayerEffect : PlayerEffect
    {
        private readonly int _amountToDraw;

        public DrawTypeAOnSpecialUnitDeathPlayerEffect(PlayerController target, ETeam creatorTeam,
        ...
        {
            if (!IsSpecialUnit(unit.Model.UnitType))
                return;
        ...
        private static bool IsSpecialUnit(EUnitType unitType)
        {
            switch (unitType)
            {
                case EUnitType.B:
